...
impl OidcServer {
	pub(crate) fn build(args: &crate::Args<'_>) -> Result<Self> {
		...
		let (signing_key_der, key_id) = match db
			...
			| Err(_) => {
				...
				db.oidc_signingkey
					.raw_put(SIGNING_KEY_DB_KEY, Cbor(&data));
				(key_der, key_id)
			},
		};
	...
	pub async fn get_client(&self, client_id: &str) -> Result<OidcClientRegistration> {
		self.db
			...
			.await
			.deserialized::<Cbor<_>>()
			.map(|cbor: Cbor<OidcClientRegistration>| cbor.0)
	...
	pub async fn take_auth_request(&self, req_id: &str) -> Result<OidcAuthRequest> {
		let request: OidcAuthRequest = self
			...
			.await
			.deserialized::<Cbor<_>>()
			.map(|cbor: Cbor<OidcAuthRequest>| cbor.0)
	...
	pub fn at_hash(access_token: &str) -> String {
		let hash = utils::hash::sha256::hash(access_token.as_bytes());
		b64.encode(&hash[..16])
	}

	#[must_use]
	pub fn auth_request_lifetime() -> Duration { AUTH_REQUEST_LIFETIME }
}
...
fn is_loopback_redirect(uri: &url::Url) -> bool {
	uri.scheme() == "http"
		&& matches!(uri.host_str().and_then(|h| h.parse::<IpAddr>().ok()), Some(ip) if ip.is_loopback())
}
