...
async fn load_joined_room(
	...
	// Note the since token is considered part of the last batch.
	let since_shortstatehash = services
		.timeline
	...
	let (since_shortstatehash, horizon_shortstatehash, current_shortstatehash, receipt_events) =
		join4(
		...
		.map(|(since, horizon, current, receipt)| {
			Ok::<_, Error>((since, horizon.flat_ok(), current?, receipt))
		})
		.boxed()
		.await?;
	...
	let lazy_load_reset: OptionFuture<_> = since_shortstatehash
		.is_none()
	...
	let (
		...
		(encrypted_room, since_sender_member, since_encryption),
	) = join3(
		witness,
		join(last_privateread_update, last_notification_read),
		join3(encrypted_room, since_sender_member, since_encryption),
	)
	.await;

	let encrypted_since_last_sync = encrypted_room && since_encryption.is_none();
	...
	let device_burst =
		joined_since_last_sync || encrypted_since_last_sync || since_shortstatehash.is_none();
	...
	let device_list_updates = state_events
		...
		.fold_default(async |(mut dlu, mut leu): pair_of!(HashSet<_>), (content, user_id)| {
			...
			match content.membership {
				...
				| Join if device_burst || !shares_encrypted_room(&user_id).await =>
					dlu.insert(user_id),
				| _ => false,
			};
...
async fn calculate_state_changes<'a>(
	...
	let initial = full_state || joined_since_last_sync || since_shortstatehash.is_none();
	...
	let incremental = !initial && since_shortstatehash != horizon_shortstatehash;
