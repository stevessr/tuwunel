...
fn default_database_path() -> PathBuf {
	nix::unistd::geteuid()
		...
		.eq(&false)
		.then(|| {
			let state_dir = std::env::var("XDG_STATE_HOME").ok()?;
			Some(format!("{state_dir}/tuwunel"))
		})
		.or_else(|| {
			let home = std::env::home_dir()?;
			let home = home.as_path().to_str()?;
			Some(format!("{home}/.local/state/tuwunel/")).map(Into::into)
		})
		.flatten()
		.map(Into::into)
		.unwrap_or_else(|| "/var/lib/tuwunel".to_owned().into())
}
